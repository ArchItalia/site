...
| `architalia`      | Proprietà dell'utente.                                                                                                                                                                                   |
| `architalia`      | Proprietà del gruppo.                                                                                                                                                                                    |
| `4096`          | Per i file, mostra la dimensione del file. Per le directory, mostra il valore fisso di 4096 byte occupati dal nome del file. Per calcolare la dimensione totale di una directory, usa `du -sh architalia/` |
| `25 oct. 08:10` | Ultima data di modifica.                                                                                                                                                                                 |
| `architalia`      | Il nome del file (o directory).                                                                                                                                                                          |
...
La directory "architalia" deve essere presente per creare la directory "work".
...
!!! Note "Nota"
    ...
    Il comando `rm` non chiede conferma quando si eliminano i file. Tuttavia, con una distribuzione arch/arch, `rm` chiede la conferma della cancellazione perché il comando `rm` è un `alias` del comando `rm -i`. Non sorprenderti se su un'altra distribuzione, come Debian, ad esempio, non ottieni una richiesta di conferma.
...
Il comando `alias` elenca gli alias per la sessione corrente. Gli alias sono stabiliti per impostazione predefinita sulle distribuzioni Linux. Qui, gli alias per un server arch Linux:
